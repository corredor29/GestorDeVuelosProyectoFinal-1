...
internal sealed class DemoReferenceDataSeeder
{
        ...
        for (var day = 0; day < 10; day++)
        {
            foreach (var slot in baseSlots)
            {
                foreach (var route in routes)
                {
                    ...
                    var dep                 = startDay.AddDays(day).AddHours(slot.Hour).AddMinutes(slot.Minute + departureMinuteOffset);
                    ...
                    if (seeds.Count + existingCount >= 80) break;
                }
                if (seeds.Count + existingCount >= 80) break;
            }
            if (seeds.Count + existingCount >= 80) break;
    ...
    public async Task SeedPairedReturnFlightsAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var f in flights.OrderBy(x => x.DepartureAt).ThenBy(x => x.RouteId))
        {
            ...
            if (revRouteId is null) continue;
            ...
            var depReturn           = f.DepartureAt.AddDays(1).AddHours(returnHourOffset).AddMinutes(returnMinuteOffset);
            ...
            if (existingDepartureKeys.Contains(key)) continue;
    ...
    // ✅ NUEVO — genera 5 vuelos futuros por cada ruta para pruebas de reprogramación
    public async Task SeedExtraFlightsPerRouteAsync(CancellationToken cancellationToken = default)
    {
        if (await _db.Flights.AsNoTracking().AnyAsync(f => f.FlightCode.StartsWith("EXT"), cancellationToken))
            return;
        ...
        if (scheduledId is null) return;

        var routes   = await _db.Routes.AsNoTracking().ToListAsync(cancellationToken);
        var airlines = await _db.Airlines.AsNoTracking().OrderBy(x => x.Id).Take(3).ToListAsync(cancellationToken);
        var aircraft = await _db.Aircrafts.AsNoTracking().OrderBy(x => x.Id).Take(3).ToListAsync(cancellationToken);

        if (routes.Count == 0 || airlines.Count == 0 || aircraft.Count == 0) return;
        ...
        foreach (var route in routes)
        {
            for (var i = 1; i <= 5; i++)
            {
                var code = $"EXT{codeSeq++}";
                var al   = airlines[i % airlines.Count];
                var ac   = aircraft[i % aircraft.Count];
                var dep  = now.AddDays(i * 2).AddHours(6 + (i * 3));
                var arr  = dep.AddMinutes(route.EstimatedDurationMin ?? 120);
